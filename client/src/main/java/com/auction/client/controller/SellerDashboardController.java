...
public class SellerDashboardController implements BidUpdateListener, AuctionUpdateListener {
    ...
    @FXML
    private Button btnTheme;  // Dark/Light mode toggle
    @FXML
    ...
    @FXML
    private Label lblClosedItems;
    @FXML
    private Label lblTotalRevenue;

    // ===== Danh sách sản phẩm (cột phải) =====
    @FXML
    private TableView<AuctionResponse> tvSellerItems;
    @FXML
    private TableColumn<AuctionResponse, Integer> colNo;
    @FXML
    private TableColumn<AuctionResponse, String> colName;
    @FXML
    private TableColumn<AuctionResponse, Double> colStart;
    @FXML
    private TableColumn<AuctionResponse, Double> colPrice;
    @FXML
    private TableColumn<AuctionResponse, Integer> colBidCount;
    @FXML
    private TableColumn<AuctionResponse, AuctionStatus> colStatus;
    @FXML
    private TableColumn<AuctionResponse, String> colEndTime;
    @FXML
    private TableColumn<AuctionResponse, Void> colAction;
    ...
    @FXML
    private Label lblTableMessage;

    // ===== Profile Popup =====
    @FXML
    private HBox headerUserArea;
    ...
    void handleAddItem(ActionEvent event) {
        try {
            ...
            if (title.isEmpty()) {
                AlertUtil.showError("Lỗi", "Vui lòng nhập tên sản phẩm.");
                return;
            }
    ...
    private void handleEditAuction(AuctionResponse auction) {
        ...
        if (auction.getCategory() != null) {
            for (ItemCategory cat : ItemCategory.values()) {
                if (cat.name().equals(auction.getCategory())) {
                    fCat.setValue(cat.getDisplayName());
                    break;
                }
            }
        }
        ...
        grid.add(fTitle, 1, row++);
        grid.add(new Label("Mô tả"), 0, row);
        grid.add(fDesc, 1, row++);
        grid.add(new Label("Danh mục"), 0, row);
        grid.add(fCat, 1, row++);
        grid.add(new Label("Giá khởi điểm *"), 0, row);
        grid.add(fPrice, 1, row++);
        grid.add(new Label("Bước giá *"), 0, row);
        grid.add(fIncr, 1, row++);

        HBox startBox = new HBox(8, fStartDate, new Label("Giờ:"), fStartHour);
        startBox.setAlignment(Pos.CENTER_LEFT);
        grid.add(new Label("Ngày bắt đầu"), 0, row);
        grid.add(startBox, 1, row++);

        HBox endBox = new HBox(8, fEndDate, new Label("Giờ:"), fEndHour);
        endBox.setAlignment(Pos.CENTER_LEFT);
        grid.add(new Label("Ngày kết thúc *"), 0, row);
        grid.add(endBox, 1, row++);
        ...
        dialog.setResultConverter(bt -> {
            ...
            try {
                incr = Double.parseDouble(fIncr.getText().trim());
            } catch (Exception ex) {
                AlertUtil.showError("Lỗi", "Bước giá không hợp lệ.");
                return null;
            }

            if (price <= 0) {
                AlertUtil.showError("Lỗi", "Giá khởi điểm phải lớn hơn 0.");
                return null;
            }
            // FIX Bug 3: cho phép bước giá = 0 (giữ nguyên giá trị cũ hoặc không có bước tối thiểu)
            if (incr < 0) {
                AlertUtil.showError("Lỗi", "Bước giá không được âm.");
                return null;
            }

            if (fEndDate.getValue() == null) {
                AlertUtil.showError("Lỗi", "Vui lòng chọn ngày kết thúc.");
                return null;
            }
            LocalTime endTime;
            try {
                endTime = LocalTime.parse(fEndHour.getText().trim(), DateTimeFormatter.ofPattern("HH:mm"));
            } catch (Exception ex) {
                AlertUtil.showError("Lỗi", "Định dạng giờ kết thúc không hợp lệ (HH:mm).");
                return null;
            }
    ...
    private StackPane createAddButton() {
        ...
        Label txt = new Label("Thêm ảnh");
        txt.setStyle("-fx-font-size:13px;-fx-font-weight:bold;-fx-text-fill:#9b59b6;");
        Label hint = new Label("(Click để chọn)");
        hint.setStyle("-fx-font-size:10px;-fx-text-fill:#aaaaaa;-fx-font-style:italic;");
